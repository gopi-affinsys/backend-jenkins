pipeline {
    agent any
    environment{
         DOCKER_IMAGE="gopikondaji/newbackend:$BUILD_NUMBER"
    }
    stages {
        stage('Checkout'){
            steps{
                checkout([$class: 'GitSCM', branches: [[name: '**/tags/**']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'git-deploy', refspec: '+refs/tags/*:refs/remotes/origin/tags/*', url: 'https://github.com/gopi-affinsys/backend-jenkins/']]])
            }
        }
    ...
    steps {
        ...
        stage("Update deploy repo"){
            environment{
                IMAGE_NAME="gopikondaji/newbackend"
                GIT_REPO="deploy"
            ...
            steps {
                ...
                withCredentials([string(credentialsId: 'test', variable: 'GITHUB_TOKEN')]) {
                    sh '''
                        ...
                        git add .
                        git commit -m "Update ${IMAGE_NAME} image version TO ${BUILD_NUMBER}"
